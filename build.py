  ...
  <div class="strengths">
    ...
    <article class="strength">
      ...
      <p>깊은 페이지 랭킹 조회가 233초까지 갔다. 집계 테이블과 스냅숏으로 뒤쪽 페이지를
        ...
        것이고, 실행계획이 그걸 보여준다. 앞 페이지에서는 오히려 느려진 것도 같이 적었다.</p>
      <a class="ref" href="#ch4">→ 4장 · 랭킹 조회</a>
    </article>
  </div>
</section>"""
...
CHAPTER_LEDES = {
    ...
    4: "읽는 경로에서 부딪힌 것. 깊은 페이지 랭킹 조회가 233초까지 갔고, 집계 테이블과 "
    "스냅숏으로 0.113ms까지 줄였다. 다만 solved 랭킹은 아직 47.7초이고, "
    "병목이 사라진 자리가 아니라 tie group 안으로 옮겨간 것이다.",
    5: "검증하지 않은 것을 먼저 적는다. 채점이 <code>Thread.sleep</code>이라는 것, "
    "재현한 장애가 프로세스 즉사 한 종류라는 것, 아직 재지 않은 부대 I/O, "
    "대회·일반 제출이 같은 큐를 쓴다는 것, 그리고 solved 랭킹의 tie group.",
}
